/**
 * llama los elementos de clientes y los almacena en respuesta.
 */
...
/**
 * llama los elementos id de cuatrimoto y los pane en respuesta.
 */
...
/**
 * valida los datos y los agrega a la tabla reservacion.
 */
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://129.151.115.74:8080/api/Reservation/save",
...
function listarReservation(){
    $.ajax({
        url:"http://129.151.115.74:8080/api/Reservation/all",
...
/**
 * ubica un elemento de la tabla reservas por el id para eliminarlo.
 * @param {*} idElemento 
 */
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.115.74:8080/api/Reservation/"+idElemento,
...
/**
 * toma los datos de la tabla y los carga para ser actualizados.
 * @param {*} id 
 */
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.115.74:8080/api/Reservation/"+id,
...
/**
 * valida la correcion de los datos y los ingresa a reservacion.
 * @param {*} idElemento 
 */
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.115.74:8080/api/Reservation/update",
